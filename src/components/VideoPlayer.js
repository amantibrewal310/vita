...
class VideoPlayer extends Component {
    ...
    */
	handleProgress = (e) => {
		...
		let tempProgressPercentage = getPercentage(
			tempProgressWidth,
			this.videoPlayer.current.clientWidth
		);
		// set progress precentage
		this.setState({
			progressPercentage: tempProgressPercentage,
		});
		this.videoRef.current.currentTime = Math.round(
			(this.state.completeDurationInSeconds * tempProgressPercentage) /
				100
		);
	};
    ...
    */
	skipForward = () => {
		...
		let timeAfterSkip = Math.min(
			currTime + 10,
			this.state.completeDurationInSeconds
		);
    ...
    */
	...
	setVolume = (e) => {
		...
		let tempVolumeValue =
			tempVolumeWidth / this.volumeProgress.current.clientWidth;
			...
			<>
				...
				<div>
							...
							<div className='video-controls'>
									...
									<div className='playback-rate'>
										...
										<div className='playback-btn'>
											<i
												...
												aria-hidden='true'
												onClick={this.increasePlayRate}
											></i>
											<i
												className='fa fa-chevron-circle-down'
												aria-hidden='true'
												onClick={this.decreasePlayRate}
											></i>
										</div>
									</div>
								</div>
								...
								<div className='right-side-controls'>
									...
									<div
										...
										onClick={this.goFullScreenOnClick}
										title='fullscreen'
									></div>
								</div>
							</div>
					...
					<div className={detailStyles.videoDetailContainer}>
						<h2 className={detailStyles.videoTitle}>
							{video.title}
						</h2>
							...
							{this.state.report ? (
								...
								<div style={{ marginTop: '10px' }}>
									<ReportVideo
										...
										toggleReportBtn={this.toggleReportBtn}
									/>
								</div>
							)}
						</div>
